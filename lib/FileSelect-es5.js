...
var FileSelect = /*#__PURE__*/function () {
  ...
  _createClass(FileSelect, [{
    ...
    value: function handleFile(file, allowedTypes) {
      ...
      return new Promise(function (resolve, reject) {
        if (_typeof(file) !== 'object' || !(file instanceof File)) reject('Invalid Argument Exception. Expected instance of file.');
        var uniqueFilename;
        if (file.name !== undefined) uniqueFilename = file.name.replace(/(?=\.[^.]+$)/, '-' + _this2.uuidv4());else uniqueFilename = Date.now();

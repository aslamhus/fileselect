 ...
 * @property {Function} options.onReadFileComplete - a client defined function to be called when the file has been read.                
 *
 */
class FileSelect {
  ...
  handleFile(file, allowedTypes) {
    return new Promise((resolve, reject) => {
      ...
      let uniqueFilename;
      if (file.name !== undefined) uniqueFilename = file.name.replace(/(?=\.[^.]+$)/, '-' + this.uuidv4());else uniqueFilename = Date.now();
      file.uuid = uniqueFilename;
      let checkType = this.checkFileTypes(file, allowedTypes);
      if (!checkType.valid) return checkType;

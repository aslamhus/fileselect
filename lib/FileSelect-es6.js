...
class FileSelect {
  ...
  async getPreview(file) {
    if (!file || typeof file !== 'object' || !(file instanceof File)) throw "Type Error. getPreview expected file object";
    let previewObj = {
      preview: null,
      icon: null
    };
    ...
    [mimetype, subtype] = file.type.split('/'); // 1. Get blobs 
    ...
    let iconBlob = this.svg[file.type] !== undefined ? await this.createSVGBlob(this.svg[file.type]) : await this.createDefaultIcon(ext, this.colors); // let iconSrc = URL.createObjectURL(iconBlob);

...
class FileSelect {
  ...
  handleFile(file, allowedTypes) {
    return new Promise((resolve, reject) => {
      ...
      const f = file; // give file a unique filename based on date

      f.uuid = file.name !== undefined ? file.name.replace(/(?=\.[^.]+$)/, `-${Date.now()}`) : Date.now(); // check filetype is allowed

      const checkType = this.checkFileTypes(file, this.allowedTypes || allowedTypes);
  ...
  async getPreview(file) {
    ...
    let blob;
    const [mimetype, subtype] = file.type.split('/'); // 1. Get blobs
    ...
    switch (mimetype) {
      case 'application':
        ...
        } else {// default for other application types
        }
      ...
      case 'text':
        ...
        } else {// default for other text types i.e. rtf
        }
    ...
    if (previewEl) previewEl.onload = URL.revokeObjectURL(url); // 4. Create icon
    ...
    const iconBlob = this.svg[file.type] !== undefined ? await FileSelect.createSVGBlob(this.svg[file.type]) : await this.fileIcon.createIcon(ext);
    const iconSrc = URL.createObjectURL(iconBlob);
    const iconEl = document.createElement('img');
    iconEl.src = iconSrc;
    iconEl.onload = URL.revokeObjectURL(iconBlob); // 5. Return preview object with preview and icon properties
  ...
  static getPDF(url) {
    return new Promise(resolve => import(
    /* webpackChunkName: "pdjs" */
    'pdfjs-dist/webpack').then(pdfjsLib => {
  ...
  static getTypeList(allowedTypes) {
    ...
    const typeArray = [];
    const messageArray = []; // push remaining types
    ...
    const message = messageArray.reduce((acc, cur, index) => {
      let msg;
      if (index === 0) msg = acc + cur;else if (index === messageArray.length - 1) msg = `${acc} or ${cur}`;else msg = `${acc}, ${cur}`;
      return msg;
    }, 'File type must be ');
    return {
      message,

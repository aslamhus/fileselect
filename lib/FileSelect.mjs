...
class FileSelect {
  ...
  async getPreview(file) {
    ...
    } else if (mimetype === 'image' || mimetype === 'video' || mimetype === 'audio' || subtype === 'pdf' || file.type === 'text/plain') {
      blob = file;
    } else {
    ...
    let icon;
    const ext = FileSelect.getExtensionFromFilename(file.name); // if there is an SVG default icon or SVG icon for the file type, use that
